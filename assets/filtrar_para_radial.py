...
# 4. Definir parâmetros de seleção
...
for cluster, group in df.groupby("clusters"):
    ...
    top_posts = group.sort_values(by="engajamento", ascending=False).head(n_select)
    selected_rows.append(top_posts)
    
    # Adiciona o cluster como nó pai (parent)
...
# 7. Criar nós dos usuários/postagens
post_nodes = []
...
final_nodes = [root_node] + clusters_info + post_nodes
...
output_path = "radial_filtrado.json"
with open(output_path, "w", encoding="utf-8") as f:
    json.dump(final_json, f, ensure_ascii=False, indent=2)

print("Arquivo radial_filtrado.json criado com sucesso!")
